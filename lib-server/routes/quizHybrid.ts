...
export async function handleQuizHybrid(req: VercelRequest, res: VercelResponse) {
  ...
  const system = [
    ...
    "아래 allowed_companies 목록에 나온 회사만 다루세요. 목록에 없는 회사명을 만들어내지 마세요.",
    "객관식 4지선다. 한국어로만 작성.",
    "【중요 — 출제 금지 유형】 사용자가 종목코드(6자리)·정확한 현재가·호가·세 자리 이상 숫자를 외워야만 맞출 수 있는 문제는 절대 내지 마세요.",
    "보기 텍스트(choices[].text)와 문제 본문(prompt)에는 6자리 종목코드만 나열하거나 ‘정확한 주가/시가총액이 얼마인가’처럼 숫자 팩트를 묻지 마세요.",
    "대신 업종, 브랜드·서비스 인지, 사업 영역, 상대적 표현(예: 소매·유통 중심인 기업은?), 시세 스니펫에 나온 등락 방향을 말로 이해하는 수준 등 일상적·개념형으로 출제하세요.",
    "JSON의 ticker 필드는 각 보기가 어떤 회사를 가리키는지 서버 검증용으로만 채우는 것이며, 사용자에게 보이는 문구에 숫자 코드를 나열하는 용도가 아닙니다.",
    "난이도: 일반 사용자가 지문만 보고도 답할 수 있을 정도로 출제하세요. 세부 재무수치·암기 위주 문제는 금지.",
    ...
    "각 선택지에는 반드시 해당 회사의 ticker(6자리 문자열)를 넣으세요. choices[].text에는 회사명·브랜드·짧은 설명 등 사람이 읽기 좋은 문구만 쓰세요(숫자 코드만 네 개 나열 금지).",
  ...
  const userPayload = {
    ...
    schema: {
      ...
      questions: [
        {
          id: "q1",
          prompt: "개념·업종·브랜드 이해형 문제 본문(종목코드·정확한 원화 금액 금지)",
          choices: [
            { key: "A", text: "회사명 또는 브랜드명으로 보기", ticker: "목록의 6자리" },
            { key: "B", text: "…", ticker: "…" },
            { key: "C", text: "…", ticker: "…" },
            { key: "D", text: "…", ticker: "…" },
          ],
